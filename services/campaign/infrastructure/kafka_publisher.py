"""Campaign Service — Kafka event publisher via Outbox Pattern.
...
class OutboxPoller:
    ...
    def __init__(self, kafka_bootstrap_servers: str) -> None:
        ...
        self._topic_map = {
            "campaign.generated": "telco.campaigns.generated",
            "campaign.ab_variant_selected": "telco.campaigns.ab_variant_selected",
            "campaign.delivered": "telco.campaigns.delivered",
        }
    ...
    async def poll_and_publish(self) -> int:
        ...
        async with AsyncSessionFactory() as session:
            # Fetch unpublished campaign events (limit 100 per poll)
            result = await session.execute(
                text("""
                    SELECT id, event_type, payload
                    FROM outbox.events
                    WHERE published = FALSE
                      AND aggregate_type = 'Campaign'
                    ORDER BY created_at
                    LIMIT 100
                """)
            )
