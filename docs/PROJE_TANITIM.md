...
| Fatura itirazı açılıyor | BillingAnalystAgent faturayı geçmiş ortalamayla karşılaştırır, anomali tespit eder, iade/ret/kısmi iade kararı verir |
| Ağ arızası tespit ediliyor | NetworkDiagnosticAgent 3 farklı persona (Analist, Mühendis, Yönetici) ile arızayı teşhis eder, gerekirse operasyona eskale eder |
...
- **Orchestrator**: Supervisor agent — gelen event'i sınıflandırır ve doğru specialist'e yönlendirir
- **4 Specialist Agent**: Her biri kendi domain'ine özel tool'larla donatılmış
...
- Her domain servisinin bir MCP server'ı var
...
> **Neden LangGraph (AutoGen değil)?** Bir framework'ü derinlemesine bilmek, ikisini yüzeysel bilmekten değerli. LangGraph'ın state machine yaklaşımı supervisor-specialist pattern'e doğal uyum sağlıyor.
...
| Teknoloji | Neden? |
|-----------|--------|
| **Keycloak** | OAuth2.0/OIDC, JWKS, RBAC (agent-operator, agent-supervisor, agent-auditor rolleri) |
| **Prompt Injection Guard** | 7 pattern kategorisi, 3 risk seviyesi — LLM'e kötü niyetli girdi gitmesini engeller |

### Altyapı & Gözlemlenebilirlik

| Teknoloji | Neden? |
|-----------|--------|
| **Docker Compose** | Tüm altyapıyı (DB, Redis, Kafka, Keycloak, monitoring) tek komutla ayağa kaldırma |
| **Traefik** | API Gateway — routing, load balancing |
| **Prometheus + Grafana** | Agent metrikleri: latency, tool çağrıları, hata oranları |
| **Jaeger** | Distributed tracing — agent → tool → DB arası izleme |
| **Loki** | Merkezi log toplama |
| **Helm Charts** | Kubernetes production deployment için hazır |
...
- **Framework showcase değildir** — Her teknoloji seçimi belirli bir problemi çözmek için yapılmıştır (aşağıdaki "Mimari Kararlar" bölümüne bakınız).
...
3. BillingAnalystAgent:
   ...
   - Karar: approve_refund | reject | partial_refund
...
agents/            → 4 specialist agent + 1 orchestrator
infrastructure/    → MCP server'lar, Docker, K8s, monitoring, Keycloak
shared/            → Base class'lar, auth, security, config, Kafka retry
scripts/           → Mock data seed, benchmark
...
Bu proje, `portfolio_roadmap.md`'deki 6 projelik serideki **ilk ve en kapsamlı proje**. Öğrettiği temel konseptler:
